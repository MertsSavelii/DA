...
void TBtree::WriteTree(TNode* curNode, std::ofstream &file) {
    for(int i = 0; i < curNode->n; ++i){
        ...
        file.write(reinterpret_cast<const char *>(&curNode->currentNode[i].value), sizeof(unsigned long long));
        file.flush();
    }
...
void TBtree::ReadTree(std::ifstream& source) {
    ...
    for(int i = 0; i < amount; ++i){
        ...
        source.read(reinterpret_cast<char *>(&value), sizeof(unsigned long long ));
...
int main(){
    ...
    while (std::cin >> command){
        if(command == "+"){
            ...
            std::cin >> key >> path;
            if(key == "Save"){
                std::ofstream fileToWrite(path, std::ios::trunc | std::ios::binary);
                int amount = 0;
                t.CountElementsTree(amount);
                fileToWrite.write(reinterpret_cast<const char *>(&amount), sizeof(int ));
                if(amount > 0) {
                    t.WriteTreeToFile(fileToWrite);
                }
                fileToWrite.close();
                std::cout << "OK" << std::endl;
            }
